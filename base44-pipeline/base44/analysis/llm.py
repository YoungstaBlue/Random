...
Real Anthropic SDK client, gated behind ``BASE44_ANTHROPIC_API_KEY``. The system
...
log = logging.getLogger("base44.analysis.llm")

_SYSTEM_PROMPT = (
    "You are the base44 legal analysis matrix. Provide STRICTLY FACTUAL legal "
...
class LegalAnalyst:
    ...
    def analyze(self, question: str, hits: List[SearchHit]) -> str:
        ...
        if not self.enabled:
            return (
                "[LLM analysis disabled — set BASE44_ANTHROPIC_API_KEY to enable]\n"
                f"Question: {question}\nTop sources: "
